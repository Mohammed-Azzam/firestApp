...
class WordPress {
  String _baseUrl = '';
  WordPressAuthenticator _authenticator = WordPressAuthenticator.JWT;
  ...
  async.Future<User> _authenticateViaJWT(
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      return fetchMeUser();
      // return fetchUser(username: authResponse.userNicename);
    } else {
      try {
  ...
  async.Future<User> authenticateViaToken(String token) async {
    ...
    Map<String, String> h = _urlHeader;
    h['Accept'] = 'application/json';
    h['Content-Type'] = 'application/json';

    final response = await http
        .post(Uri.parse(_baseUrl + URL_JWT_TOKEN_VALIDATE), headers: h);
  ...
  async.Future<User> fetchUser(
    ...
    final Map<String, String> params = {'search': '', 'context': 'edit'};
    if (id != null) {
      params['search'] = '$id';
    } else if (email != null)
      params['search'] = email;
    ...
    final response =
        await http.Client().get(Uri.parse(url.toString()), headers: h);
  ...
  async.Future<User> fetchMeUser() async {
    ...
    final Map<String, String> params = {'context': 'edit'};
    url.write(constructUrlParams(params));
    Map<String, String> h = _urlHeader;
    h['Accept'] = 'application/json';
    h['Content-Type'] = 'application/json';

    final response = await http.get(Uri.parse(url.toString()), headers: h);
  ...
  async.Future<List<Post>> fetchPosts({
    ...
    print('fetching posts: '+url.toString());
  ...
  async.Future<Post> createPost({required Post post}) async {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      try {
        ...
        throw new WordPressError(message: json.decode(response.body));
      }
    }
  }
